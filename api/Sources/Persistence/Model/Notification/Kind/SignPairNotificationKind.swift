...
final class SignPairNotificationKind: Model {

    static let schema = "sign_pair_notification_kinds"
...
extension Entity.NotificationKind {
    func toSignPairModel(of pairNotificationID: PairNotification.IDValue)
        -> SignPairNotificationKind?
    {
        guard case .sign(let task) = self else {
            return nil
        }
        return .init(
...
extension SignPairNotificationKind {
